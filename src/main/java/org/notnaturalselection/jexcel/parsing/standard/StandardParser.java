...
public class StandardParser<T> extends ReflectionParser<T> {
    ...
    public static class Builder<T> extends ReflectionParser.Builder<Builder<T>, T> {
        ...
        public Builder(Iterable<Header> headers, Supplier<T> supplier) {
    ...
    private StandardParser(
            WarningPolicy wp, int verticalOffset, int horizontalOffset, Supplier<T> supplier, Iterable<Header> headers
    )
            throws FieldMappingException {
        super(verticalOffset, horizontalOffset, wp, supplier);
        setFieldMapping(generateMapping(supplier.get().getClass(), headers));
    }

    private static Map<Header, Field> generateMapping(Class<?> aClass, Iterable<Header> headers)
            throws FieldMappingException {
        ...
        for (Header header : headers) {
            try {
                ...
                throw new FieldMappingException(aClass.toString() + " has no field with name " + header.getColumnName());
            }
        }
        return fieldMapping;
    }
